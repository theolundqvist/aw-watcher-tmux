...
def filter_work(events: List[dict], repo) -> List[dict]:
    res = []
    for i, e in enumerate(events):
        if 'repo_url' in e['data']:
            if e['data']['repo_url'] == repo:
               res.append(e) 
        else: continue; 
    return res;
...
def query(regex: str = EXAMPLE_REGEX, save=False, repo_url=None):
    ...
    if repo_url is not None:
        for i, (start, stop) in enumerate(timeperiods):
            res[i]['events'] = filter_work(res[i]['events'], repo_url)#[e for e in res[i]['events'] if 'repo_url' in e and e['repo_url'] == repo]
...
if __name__ == "__main__":
    if(len(sys.argv) > 1):
        ...
        res = query(save=True, repo_url=sys.argv[1])
        print("res:", res)
    else:
        res = query(save=True)
        print("res:", res)
